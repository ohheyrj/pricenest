    ...
    try:
        ...
        try:
            ...
            if not headers or 'title' not in headers:
                return jsonify({'error': 'CSV must have at least a "title" column. Optional: "director", "year"'}), 400
        ...
        for result in preview_results:
            if result['status'] == 'pending':
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?)
                ''', (category_id, csv_data['title'], csv_data['director'], csv_data['year'], 
...
def process_pending():
    ...
    try:
        ...
        for search in pending_searches:
            search_id, category_id, title, director, year, csv_row_data, retry_count = search
...
def add_manual_movie():
    ...
    try:
        ...
        if not category_id or not title:
            return jsonify({'error': 'Missing required fields: category_id and title'}), 400
        ...
        try:
            ...
            return jsonify({'error': 'Invalid data types for category_id, year, or price'}), 400
